...
class ControllerCommonHome extends Controller
{
	public function index()
	{
		...
		$courses_and_workshops = $this->model_catalog_category->getCategories(72);
		...
		foreach ($results as $result) {
			if ($result['image']) {
				$image = $result['image']; // $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
			} else {
			...
			$data['testimonials'][] = array(
				...
				'thumb'       => $image,
				'name'        => $result['name'],
				'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, strlen($result['description'])),
				'price'       => $price,
				'special'     => $special,
				'tax'         => $tax,
